...
export default function ListOrders({ data }) {
  ...
  const columns = ["ID", "Order Date", "Employee", "Total Cost", "Confirmed"];
  ...
  useEffect(async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/orders/getOrderedParts`
    );
    const json = await res.json().then((data) => setOrderedParts(data.orders));
  }, []);

  const confirm = (orderID, isConfirmed) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/updateConfirm`, {
      method: "PUT",
            ...
            <ListItem sizes={columnSizes} key={item.orderID}>
              ...
              <ListItem.Content>
                <div className={styles.content}>
                  ...
                  <button
                    onClick={() =>
                      confirm(item.orderID, item.isConfirmed ? 0 : 1)
                    }
                  >
